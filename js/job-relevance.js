...
(function (root, factory) {
  ...
  const economic = /gesundheits[oö]konom|gesundheitsoekonom|health econom|pharma[oö]konom|pharmacoeconom|\bheor\b|health technology assessment|\bhta\b|cost[- ]effectiveness|kosten[- ](?:wirksamkeit|nutzen)|economic evaluation|budget impact/;
  ...
  const healthDomain = /gesundheits(?:wesen|system|versorgung|politik|daten|bereich|wissenschaft|forderung|management|okonom)|health ?care|health (?:system|services|data|management|econom|polic)|medical data|klinische(?:n)? (?:daten|prozesse)|medizinische(?:n|r)? (?:daten|versorgung)|patient(?:en|innen)?(?:versorgung|sicherheit|prozesse|pfade)|spital(?:planung|finanzierung|prozesse|organisation)|krankenversicherung|soins de sante|systeme de sante|ehealth|e-health|digisante|klinikinformationssystem|\bzkis\b/;
  const researchRole = /wissenschaft|research|forsch|phd|doctoral|postdoc|doktorand|epidemiolog|statisti|analyst|analyse|spezialist|fachperson|referent|praktik|consultant|berater/;
  ...
  function jobSections(job) {
    ...
    const cutoff = /(?:^|\n)\s*(?:angebot|unser angebot|wir bieten(?: ihnen| dir)?|warum [^\n]{1,60}|was wir (?:dir|ihnen) bieten|das durfen sie von uns erwarten|darauf konnen sie sich freuen|deine vorteile|ihre vorteile|benefits|uber den arbeitgeber|(?:\w+ ){0,4}als arbeitgeber(?:in)?|kontakt|weitere informationen)\s*(?:\n|:|$)/m;
    const body = full.split(cutoff)[0];
    const requirementHeading = /(?:^|\n)\s*(?:anforderungen|ihr profil|dein profil|das bring(?:en sie|st du) mit|das zeichnet dich aus|was sie mitbringen|was sie auszeichnet|your profile|requirements|qualifications)\s*(?:\n|:|$)/m;
    const pieces = body.split(requirementHeading);
    let tasks = pieces[0];
    const taskHeading = /(?:^|\n)\s*(?:aufgaben|ihre aufgaben|deine aufgaben|das sind deine aufgaben|ihr verantwortungsbereich|ihre neue herausforderung|was sie bewegen|so gestaltest du gesundheit mit|your responsibilities)\s*(?:\n|:|$)/m;
